...
# 센서 핀 설정 (각 구역당 2칸만 사용)
sensor_pins = {
    'T1A': [{'TRIG': 23, 'ECHO': 24}, {'TRIG': 27, 'ECHO': 22}],
    'T1B': [{'TRIG': 5,  'ECHO': 6},  {'TRIG': 17, 'ECHO': 18}],
    'T2A': [{'TRIG': 12, 'ECHO': 16}, {'TRIG': 20, 'ECHO': 21}],
    'T2B': [{'TRIG': 13, 'ECHO': 19}, {'TRIG': 26, 'ECHO': 4}],
    '전기차': [{'TRIG': 25, 'ECHO': 8}, {'TRIG': 7,  'ECHO': 1}],
    '장애인': [{'TRIG': 9,  'ECHO': 10}, {'TRIG': 11, 'ECHO': 14}]
}

# 주차 상태 및 수동 배정 플래그 초기화
parking_zones = {zone: [0] * len(sensors) for zone, sensors in sensor_pins.items()}
manual_override = {zone: [False] * len(sensors) for zone, sensors in sensor_pins.items()}
...
def find_empty_spot(zone_name):
    for i in range(len(parking_zones[zone_name])):
...
def assign_parking(car_type):
    ...
    if car_type == WHITE:
        ...
        for zone in preferred_order + ['T2A', 'T2B']:
            ...
            if spot is not None:
                ...
                manual_override[zone][spot] = True  # 수동 배정 설정
                assigned_zone = zone
                assigned_spot = spot
                assigned = True
                break
    ...
    if assigned:
        ...
        print(f"📊 현재 '{assigned_zone}' 주차 현황: {empty_count}/{len(current_status)} 자리 비어 있음 -> {current_status}\n")
    else:
...
def remove_parking(zone_name, spot_number):
    ...
    if parking_zones[zone_name][spot_index] == 0:
        ...
        manual_override[zone_name][spot_index] = False  # 수동 배정 해제
        print(f"✅ '{zone_name}' 구역 {spot_number}번 자리 차량이 출차되었습니다.")
        current_status = parking_zones[zone_name]
        empty_count = current_status.count(0)
        print(f"📊 '{zone_name}' 주차 현황: {empty_count}/{len(current_status)} 자리 비어 있음 -> {current_status}\n")

def print_parking_status():
    print("\n📡 센서 거리 측정 중...")
    for zone in sensor_pins:
        empty_count = parking_zones[zone].count(0)
        print(f"🅿️ {zone}: {parking_zones[zone]} (빈 자리 {empty_count}/{len(parking_zones[zone])})")
    print()
    ...
    while True:
        ...
        for zone in sensor_pins:
            for i, sensor in enumerate(sensor_pins[zone]):
                ...
                # 수동 배정 시 센서 무시하고 1 유지
                if manual_override[zone][i]:
                    parking_zones[zone][i] = 1
                    if dist <= 25:
                        manual_override[zone][i] = False  # 차량 도착했으므로 해제
                else:
